function quiverC2D_Scaled(x,y,u,v,varargin)
...
addOptional(p, 'scale', 8, @isscalar); %5
addOptional(p, 'MaxNumArrows', inf, @isscalar);
addParameter(p, 'Normalize', false);
addParameter(p, 'LineWidth', 2, @isscalar); %5
addParameter(p, 'MaxHeadSize', 2, @isscalar); %2
...
ylim auto;
%  xlim(ax,[0 0.0063648]);
%  ylim(ax,[0 0.00452608]);
%  xlim(ax,[0 0.004234]);
%  ylim(ax,[0 0.00311]);
%cbh = colorbar; set(cbh,'FontSize',18,'FontWeight','bold');hold on
if numel(u) > ncolors
...
if p.Results.Colorbar
    ...
    %tlabels = sprintfc('%1.3g ',linspace(3.66e-05, 7.23e-05, length(tlabels)));
