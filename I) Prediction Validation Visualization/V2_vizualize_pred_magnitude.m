function V2_vizualize_pred_magnitude(maspath,T1,revimg,imgtitle,ylbl,blocksize)
...
%ET = normalize(EET)
yfit = T1.trainedModel_Optimized_RGPR.predictFcn(ET);
% DEFAULT:
[~,~,idx] = histcounts(yfit);
...
ET_Head = yfit(1:length(ET1.Magnitude),:);
%ET_Head(ind1)=0;

ET_Mu = yfit(length(ET1.Magnitude)+1:length(ET1.Magnitude)+length(ET2.Magnitude),:);
%ET_Mu(ind2)=0;

ET_Md = yfit(length(ET1.Magnitude)+length(ET2.Magnitude)+1:length(ET1.Magnitude)+length(ET2.Magnitude)+length(ET3.Magnitude),:);
%ET_Md(ind3)=0;

ET_Tail = yfit(length(ET1.Magnitude)+length(ET2.Magnitude)+length(ET3.Magnitude)+1:end,:);
%ET_Tail(ind4)=0;
