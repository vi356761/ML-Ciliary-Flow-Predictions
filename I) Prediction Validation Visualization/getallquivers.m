function getallquivers(maspath,pathy,blksize,T1,T2,pthnme)
MM = load([maspath,'Extracted_All_MAT_Files.mat']);
tt=load([pathy,'Sectioned_Velocity.mat']);

%Original orientation
canvas1 = tt.Org.Sections2.Head;
canvas2 = tt.Org.Sections2.Tail;
canvas3 = tt.Org.Sections2.Midup;
canvas4 = tt.Org.Sections2.Middown;
...
ux1 = G_ET_Head.*cosd(G_EET_Head); uy1 = G_ET_Head.*sind(G_EET_Head); ux1_pred = G_ET_Head_Pred.*cosd(G_EET_Head_Pred); uy1_pred = G_ET_Head_Pred.*sind(G_EET_Head_Pred);
ux2 = G_ET_Tail.*cosd(G_EET_Tail); uy2 = G_ET_Tail.*sind(G_EET_Tail); ux2_pred = G_ET_Tail_Pred.*cosd(G_EET_Tail_Pred); uy2_pred = G_ET_Tail_Pred.*sind(G_EET_Tail_Pred);
ux3 = G_ET_Midup.*cosd(G_EET_Midup); uy3 = G_ET_Midup.*sind(G_EET_Midup); ux3_pred = G_ET_Midup_Pred.*cosd(G_EET_Midup_Pred); uy3_pred = G_ET_Midup_Pred.*sind(G_EET_Midup_Pred);
ux4 = G_ET_Middown.*cosd(G_EET_Middown); uy4 = G_ET_Middown.*sind(G_EET_Middown); ux4_pred = G_ET_Middown_Pred.*cosd(G_EET_Middown_Pred); uy4_pred = G_ET_Middown_Pred.*sind(G_EET_Middown_Pred);
%% Plot figures
