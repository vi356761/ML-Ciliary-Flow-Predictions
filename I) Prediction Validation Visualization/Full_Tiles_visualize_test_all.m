function Full_Tiles_visualize_test_all(n_sqr_trop,pdsz_trop,M_Test_Trop,Trop_Y,Raw_Test_Trop,pathysv,nmesv)
...
for yy=1:length(Trop_Y2)
    L = zeros(size(n_sqr_trop,n_sqr_trop));
    L(1:n_sqr_trop,1:n_sqr_trop) = idx(yy);    %fill in indices
    binind{yy,1}=L;
end
...
for yy=1:length(Trop_Y2)
    L = zeros(size(n_sqr_trop,n_sqr_trop));
    L(1:n_sqr_trop,1:n_sqr_trop) = idxx(yy);    %fill in indices
    binind2{yy,1}=L;
end
%% Index of difference
...
for yy=1:length(Trop_Y2)
    L = zeros(size(n_sqr_trop,n_sqr_trop));
    L(1:n_sqr_trop,1:n_sqr_trop) = iidx(yy);    %fill in indices
    ibinind{yy,1}=L;
end
%% Ind images:
Master_Trop_Ind=[];
...
saveas(figure1,[pathysv,'Predicted_Tiles_',nmesv,'.tif'])
...
axes1.YColor = k;%
set(gcf,'Position',get(0,'Screensize'))
saveas(figure2,[pathysv,'Observed_Tiles_',nmesv,'.tif'])
...
axes1.YColor = k;%
set(gcf,'Position',get(0,'Screensize'))
saveas(figure2,[pathysv,'Absolute_Difference_Tiles_',nmesv,'.tif'])
end